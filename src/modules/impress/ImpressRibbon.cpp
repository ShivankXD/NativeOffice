...
QWidget* ImpressRibbon::buildInsertTab() {
    ...
    auto* btnText = makeToolBtn("T", "Text Box", true);
    auto* btnRect = makeToolBtn("▭", "Rectangle", true);
    auto* btnEllipse = makeToolBtn("◯", "Ellipse", true);
    ...
    layout->addWidget(makeGroup("Shapes", { btnText, btnRect, btnEllipse }));
    layout->addWidget(makeSeparator());

    auto* btnImage = makeCmdBtn(QStringLiteral("\U0001F5BC  Picture"), "Insert Image from file", 96);
    connect(btnImage, &QToolButton::clicked, this, &ImpressRibbon::insertImageRequested);
    layout->addWidget(makeGroup("Media", { btnImage }));
...
QWidget* ImpressRibbon::buildDesignTab() {
    ...
    auto* btnCustom = makeCmdBtn("⊕  Custom…", "Pick a custom background colour", 90);
    ...
    layout->addWidget(makeGroup("Slide Background", { swatchRow, btnCustom }));
    layout->addStretch();
    return tab;
}
...
QWidget* ImpressRibbon::buildTransitionsTab() {
    ...
    struct TItem { const char* label; SlideTransition t; };
    const TItem items[] = {
        { "None", SlideTransition::None }, { "Fade", SlideTransition::Fade },
        { "Push", SlideTransition::Push }, { "Wipe", SlideTransition::Wipe },
        { "Zoom", SlideTransition::Zoom },
    };
    QList<QWidget*> tBtns;
    for (const auto& it : items) {
        auto* b = makeCmdBtn(it.label, QString("%1 transition").arg(it.label), 62, true);
    ...
    auto* btnAll = makeCmdBtn("⧉  Apply to All", "Apply this transition to every slide", 120);
...
QWidget* ImpressRibbon::buildAnimationsTab() {
    ...
    struct AItem { const char* label; const char* tip; ItemAnimation a; };
    const AItem items[] = {
        { "None",     "No entrance animation",            ItemAnimation::None },
        { "Fade In",  "Object fades in",                  ItemAnimation::FadeIn },
        { "Fly In",   "Object flies in from the left",    ItemAnimation::FlyInLeft },
        { "Zoom In",  "Object zooms in from its centre",  ItemAnimation::ZoomIn },
    };
    QList<QWidget*> aBtns;
    for (const auto& it : items) {
        auto* b = makeCmdBtn(it.label, it.tip, 76);
